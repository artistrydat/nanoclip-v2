...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@nanoclip/adapter-utils";
...
export interface AdapterConfigFieldsProps {
  ...
  mark: (group: "adapterConfig", field: string, value: any) => void;
...
export interface UIAdapterModule {
  type: string;
  label: string;
  parseStdoutLine: (line: string, ts: string) => import("@nanoclip/adapter-utils").TranscriptEntry[];
  ConfigFields: ComponentType<AdapterConfigFieldsProps>;
  buildAdapterConfig: (values: CreateConfigValues) => Record<string, any>;
}
